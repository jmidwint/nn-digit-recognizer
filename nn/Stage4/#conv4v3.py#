...
VALIDATION_SIZE = 5000  # Size of the validation set. 
                        #jkm - look at this , should use percentage of total 
...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    if FLAGS.self_test:
        ...
        m_test = 10000
        test_data = extract_data(test_data_filename, m_test)
        test_labels = extract_labels(test_labels_filename, m_test)
    ...
    # For the validation, we'll just hold the entire dataset in
    # one constant node. (jkm -both val and test were in constant node0
    validation_data_node = tf.constant(validation_data)
    # test_data_node = tf.constant(test_data)

    # jkm - put the test data into a batch node, make it a variable size tensor batch
    # jkm - kluge here, lets set kludge batch size to 100 so it goes into
    #        the total number of 10000 test examples evenly
    BATCH_TEST_DATA_SIZE = 100 
    batch_test_data_node = tf.placeholder(tf.float32,
                            shape=(BATCH_TEST_DATA_SIZE, IMAGE_SIZE, IMAGE_SIZE, NUM_CHANNELS))


    # jkm - create a batch style node for the kaggle test data
    # jkm - Rather then setting up variable tensor flow batch sizes, will
    #       just make sure that the number of samples can be divided into an 
    #       equal size set of batches. (a bit of a kludge here).
    KAGGLE_BATCH_SIZE = 100 # since we know kaggle test size is 28000 
    batch_kaggle_test_data_node = tf.placeholder(tf.float32,
    ...
    # Predictions for the minibatch, validation set
    train_prediction = tf.nn.softmax(logits)
    # We'll compute them only once in a while by calling their {eval()} method.
    validation_prediction = tf.nn.softmax(model(validation_data_node))


    # jkm - Create a prediction node for the test set being done in batches
    #     This is needed to be done in batches since some target processor 
    #     memory cannot handle loading the entire set at once into memory
    batch_test_prediction = tf.nn.softmax(model(batch_test_data_node))

    # jkm - Create a prediction node for the kaggle test set being done in batches
    batch_kaggle_test_prediction = tf.nn.softmax(model(batch_kaggle_test_data_node))
    
    # jkm - used to calculate when to calculate results on major epochs
    epoch_step_int = (int(train_size / BATCH_SIZE))
    epoch_step_total = (int(num_epochs * train_size / BATCH_SIZE)) 
    ...
    with tf.Session() as s:
        ...
        for step in xrange(int(num_epochs * train_size / BATCH_SIZE)):
            ...
            if step % 100 == 0:
                ...
                print 'Minibatch error: %.4f%%' % error_rate(predictions,
                                                             batch_labels)
                print 'Validation error: %.4f%%' % error_rate(
                    validation_prediction.eval(), validation_labels)
                sys.stdout.flush()
            # jkm - Calculate some results every major epoch
            if step % epoch_step_int == 0:
                major_epoch = (float(step) * BATCH_SIZE / train_size)
                print 'Major Epoch %.2f' % major_epoch
                print 'Calculating Test Error Result at this epoch'
                sys.stdout.flush()
                test_result = numpy.zeros((m_test, NUM_LABELS))
                test_size = m_test 
                for test_step in xrange(test_size /BATCH_TEST_DATA_SIZE):
                    offset = test_step * BATCH_TEST_DATA_SIZE
                    batch_data = test_data[offset:(offset + BATCH_TEST_DATA_SIZE), :, :, :]
                    feed_dict = { batch_test_data_node: batch_data }
                    (batch_predictions,) = s.run([batch_test_prediction], feed_dict=feed_dict)
                    test_result[offset:(offset + BATCH_TEST_DATA_SIZE)] = batch_predictions        
                test_error = error_rate(test_result, test_labels)
                print 'At Epoch: %.2f, Test error: %.4f%%' % (major_epoch, test_error)
                # Continue if we are still getting improvements of the test data
                # jkm - add in the continue part later
                # jkm - Get the Kaggle test results, need to use the kaggle batch node/model
                # jkm - TODO: add a check to make sure samples go into batches
                print 'Calculate an interim Kaggle result for this epoch'
                sys.stdout.flush()      
                kaggle_result = numpy.zeros(m_KaggleTest)
                k_test_size = m_KaggleTest 
                for k_step in xrange(k_test_size / KAGGLE_BATCH_SIZE):
                    offset = k_step * KAGGLE_BATCH_SIZE 
                ...
                # jkm - write the interim results
                # Create a filname to use to write the results
                fn = 'Interim_CNN_Num_Epochs_{0}_Last_LR_{1}_Test_Error_{2}'.format(
                        round(major_epoch), lr, test_error)
                jkm_writeKaggleTestResults(fn, kaggle_result)
                sys.stdout.flush() # jkm - there is a print statement in this call


        # Finally get some of the result!
        # jkm - don't process all the test results at once but do it in batches
        # jkm - make sure to have chosen a batch size that goes evenly into
        #        the total so as not to leave any samples unprocessed.  
        test_result = numpy.zeros((m_test, NUM_LABELS))
        test_size = m_test 
        for step in xrange(test_size /BATCH_TEST_DATA_SIZE):
            offset = step * BATCH_TEST_DATA_SIZE
            batch_data = test_data[offset:(offset + BATCH_TEST_DATA_SIZE), :, :, :]
            feed_dict = { batch_test_data_node: batch_data }
            (batch_predictions,) = s.run([batch_test_prediction], feed_dict=feed_dict)
            test_result[offset:(offset + BATCH_TEST_DATA_SIZE)] = batch_predictions        
        ...
        # jkm - Get the Kaggle test results, need to use the kaggle batch node/model
        #     - don't process all the kaggle test results at once but do it in batches
        #     - make sure to have chosen a batch size that goes evenly into
        #        the total so as not to leave any samples unprocessed.  
        kaggle_result = numpy.zeros(m_KaggleTest)
        k_test_size = m_KaggleTest 
        for step in xrange(k_test_size / KAGGLE_BATCH_SIZE):
